window._my_educacion = [
	{
		nivel: 'Bachillerato',
		nivelIN: 'High school',
		escuela: 'CBTis No. 222',
		link: 'https://cbtis222.edu.mx/',
	},{
		nivel: 'Bachillerato',
		nivelIN: 'High school',
		escuela: 'CEB 5/4, Profesor Rafael Ramírez',
		link: 'https://ceb54.online/',
	},{
		nivel: 'Universidad',
		nivelIN: 'University',
		escuela: 'Instituto Tecnológico de Pachuca',
		link: 'https://itp.itpachuca.edu.mx/',
	},{
		nivel: 'Cursos: React, NODE, React Navive, Git + GitHub',
		nivelIN: 'Courses: React, NODE, React Navive, Git + GitHub',
		escuela: 'UDEMY',
		link: 'https://www.udemy.com/',
	},{
		nivel: 'Certificado Google Cloud Computing Foundations',
		nivelIN: 'Google Cloud Computing Foundations Certificate',
		escuela: 'Google',
		link: 'https://www.credly.com/badges/cacc1a81-366d-482d-8cad-7b9bb918303c/public_url',
	},{
		nivel: 'Bases Tecnológicas para el Servidor Público',
		nivelIN: 'Bases Tecnológicas para el Servidor Público',
		escuela: 'Gobierno de México',
		link: 'https://www.epc.gob.mx/acreditacion/EDC-HquRFF',
	},
];

window._my_experiencia = [
	{
		desc: 'Diseñador gráfico e impresor en "Impresos Brenda". (2007 - 2011)',
		descIN: 'Graphic designer and printer at "Impresos Brenda". (2007 - 2011)',
		link: '',
	},{
		desc: 'Cajero en "MEGA Comercial Mexicana". (2008 - 2009)',
		descIN: 'Cashier at "MEGA Comercial Mexicana". (2008 - 2009)',
		link: '',
	},{
		desc: 'Administrador general, diseñador e impresor en "Esmagenta". (2014 - 2018)',
		descIN: 'General administrator, designer and printer at "Esmagenta". (2014 - 2018)',
		link: '',
	},{
		desc: 'Desarrollo web en "A+ Group Consulting" y "FireCom". (2021 - 2025)',
		descIN: 'Web development at "A+ Group Consulting" and "FireCom". (2021 - 2025)',
		link: 'https://agc.com.mx/',
	},{
		desc: 'Desarrollo de sistemas y soporte técnico en "Call Center MetLife - Grupo Invictus". (2020 - 2025)',
		descIN: 'Systems development and technical support in the "Call Center MetLife - Grupo Invictus". (2020 - 2025)',
		link: 'https://metlife.grupoinviktus.com',
	},{
		desc: 'Diseño, programación y soporte infórmatico en la "Secretaría de Desarrollo Económico". (2013 - 2025)',
		descIN: 'Design, programming and computer support in the "Secretaría de Desarrollo Económico". (2013 - 2025)',
		link: 'http://sedeco.hidalgo.gob.mx',
	},
];

window._my_tecnicas = [
	{
		nombre: "HTML / CSS",
		porcentaje: "95",
		posicion: 1,
	},{
		nombre: "Bootstrap / Tailwind",
		porcentaje: "75",
		posicion: 1,
	},{
		nombre: "React JS",
		porcentaje: "90",
		posicion: 1,
	},{
		nombre: "Next JS",
		porcentaje: "35",
		posicion: 1,
	},{
		nombre: "React Native",
		porcentaje: "65",
		posicion: 1,
	},{
		nombre: "Angular / Vue",
		porcentaje: "20",
		posicion: 1,
	},{
		nombre: "Node / Express",
		porcentaje: "60",
		posicion: 1,
	},{
		nombre: "PHP",
		porcentaje: "90",
		posicion: 1,
	},{
		nombre: "Java",
		porcentaje: "30",
		posicion: 1,
	},{
		nombre: "Python",
		porcentaje: "25",
		posicion: 1,
	},{
		nombre: "JavaScript",
		porcentaje: "85",
		posicion: 1,
	},{
		nombre: "TypeScript",
		porcentaje: "55",
		posicion: 1,
	},{
		nombre: "MySQL",
		porcentaje: "90",
		posicion: 1,
	},{
		nombre: "PostgreSQL",
		porcentaje: "80",
		posicion: 1,
	},{
		nombre: "Mongo DB",
		porcentaje: "65",
		posicion: 1,
	},{
		nombre: "Apache / Nginx",
		porcentaje: "65",
		posicion: 1,
	},{
		nombre: "Git & GitHub",
		porcentaje: "70",
		posicion: 1,
	},{
		nombre: "Figma",
		porcentaje: "60",
		posicion: 2,
	},{
		nombre: "Visual Studio Code",
		porcentaje: "85",
		posicion: 2,
	},{
		nombre: "Postman",
		porcentaje: "70",
		posicion: 2,
	},{
		nombre: "Adroid Studio",
		porcentaje: "55",
		posicion: 2,
	},{
		nombre: "Google Cloud",
		porcentaje: "50",
		posicion: 2,
	},{
		nombre: "QGis",
		porcentaje: "75",
		posicion: 2,
	},{
		nombre: "Camtasia Studio",
		porcentaje: "95",
		posicion: 2,
	},{
		nombre: "Photoshop",
		porcentaje: "90",
		posicion: 2,
	},{
		nombre: "Illustrator",
		porcentaje: "90",
		posicion: 2,
	},{
		nombre: "Flash / Edge Animate",
		porcentaje: "70",
		posicion: 2,
	},{
		nombre: "Premiere",
		porcentaje: "60",
		posicion: 2,
	},{
		nombre: "Filmora",
		porcentaje: "60",
		posicion: 2,
	},{
		nombre: "After Effects",
		porcentaje: "45",
		posicion: 2,
	},{
		nombre: "Microsft Office",
		porcentaje: "95",
		posicion: 2,
	},{
		nombre: "Corel Draw",
		porcentaje: "85",
		posicion: 2,
	},{
		nombre: "Windows",
		porcentaje: "95",
		posicion: 2,
	},{
		nombre: "Linux",
		porcentaje: "80",
		posicion: 2,
	},{
		nombre: "Mac OS",
		porcentaje: "75",
		posicion: 2,
	},
];

window._my_personales = [
	{
		nombre: "Creatividad",
		nombreIN: "Creativity",
		porcentaje: 4,
		posicion: 1,
	},{
		nombre: "Compromiso social",
		nombreIN: "Social commitment",
		porcentaje: 5,
		posicion: 1,
	},{
		nombre: "Comunicación",
		nombreIN: "Communication",
		porcentaje: 3,
		posicion: 1,
	},{
		nombre: "Liderazgo",
		nombreIN: "Leadership",
		porcentaje: 4,
		posicion: 1,
	},{
		nombre: "Lógica",
		nombreIN: "Logic",
		porcentaje: 5,
		posicion: 2,
	},{
		nombre: "Organización",
		nombreIN: "Organization",
		porcentaje: 4,
		posicion: 2,
	},{
		nombre: "Responsabilidad",
		nombreIN: "Responsibility",
		porcentaje: 5,
		posicion: 2,
	},{
		nombre: "Trabajo en equipo",
		nombreIN: "Teamwork",
		porcentaje: 4,
		posicion: 2,
	},
];

const hobbies = [
	{
		nombre: "Video juegos",
		nombreIN: "Video games",
		img: "img/hobbies/juegos.png"
	},{
		nombre: "Tecnología",
		nombreIN: "Technology",
		img: "img/hobbies/tecnologia.png"
	},{
		nombre: "Correr",
		nombreIN: "Run",
		img: "img/hobbies/correr.png"
	},{
		nombre: "Gym",
		nombreIN: "Gym",
		img: "img/hobbies/gym.png"
	},{
		nombre: "Viajar",
		nombreIN: "Travel",
		img: "img/hobbies/viajar.png"
	},{
		nombre: "Juegos de mesa",
		nombreIN: "Table games",
		img: "img/hobbies/ajedrez.png"
	},{
		nombre: "Ver y jugar futbol",
		nombreIN: "Watch and play soccer",
		img: "img/hobbies/futbol.png"
	},{
		nombre: "Comer",
		nombreIN: "Eat",
		img: "img/hobbies/comer.png"
	},{
		nombre: "Escuchar música",
		nombreIN: "Listen to music",
		img: "img/hobbies/musica.png"
	},{
		nombre: "Salir con mis amigos",
		nombreIN: "Out with my friends",
		img: "img/hobbies/amigos.png"
	},{
		nombre: "Bailar",
		nombreIN: "Dance",
		img: "img/hobbies/bailar.png"
	},{
		nombre: "Ir al cine",
		nombreIN: "Go to the cinema",
		img: "img/hobbies/cine.png"
	},{
		nombre: "Nadar",
		nombreIN: "Swim",
		img: "img/hobbies/nadar.png"
	},{
		nombre: "Andar en bicicleta",
		nombreIN: "Riding a bicycle",
		img: "img/hobbies/bicicleta.png"
	},{
		nombre: "Conducir el auto",
		nombreIN: "Drive the car",
		img: "img/hobbies/manejar.png"
	},{
		nombre: "Perros",
		nombreIN: "Dogs",
		img: "img/hobbies/mascota.png"
	},{
		nombre: "Ver anime",
		nombreIN: "Watch Anime",
		img: "img/hobbies/anime.png"
	},{
		nombre: "Lindsey Stirling",
		nombreIN: "Lindsey Stirling",
		img: "img/hobbies/lindsey.png"
	},{
		nombre: "Batman",
		nombreIN: "Batman",
		img: "img/hobbies/batman.png"
	}
];

hobbies.sort((a, b) => {
    return a.nombre.localeCompare(b.nombre);
})

window._my_hobbies = hobbies

window._my_portafolio = [
	{
		id: 1,
		img: 'img/p_1.svg',
		categoria: 'Sistemas/Sitios',
		categoriaIN: 'Systems/Sites',
		proyecto: [
			{
				id: 1,
				foto: 'img/otros/investh.gif',
				link: 'http://investhidalgo.mx',
				titulo: 'Invest Hidalgo',
				descripcion: 'Invest Hidalgo',
			},{
				id: 2,
				foto: 'img/otros/c21.gif',
				link: 'https://cambio21.com.mx/',
				titulo: 'Cambio 21',
				descripcion: 'Noticias',
			},{
				id: 3,
				foto: 'img/otros/mapa.gif',
				link: 'https://mapa.investhidalgo.mx/index.html',
				titulo: 'Mapa Económico',
				descripcion: 'Mapa Económico',
			},{
				id: 21,
				foto: 'img/otros/llano.gif',
				link: 'https://llano.agc.com.mx/',
				titulo: 'Llano Norte',
				descripcion: 'Llano Norte',
			},{
				id: 27,
				foto: 'img/otros/boda.gif',
				link: 'https://miboda.agc.com.mx/',
				titulo: 'MI BODA',
				descripcion: 'BODA FABIOLA & ISRAEL',
			},{
				id: 25,
				foto: 'img/otros/calavera.gif',
				link: 'https://lostrescalavera.com/',
				titulo: 'LOS TRES CALAVERA',
				descripcion: 'LOS TRES CALAVERA',
			},{
				id: 22,
				foto: 'img/otros/hgo.gif',
				link: 'https://energiaparaelbienestar.hidalgo.gob.mx/',
				titulo: 'Energía para el bienestar',
				descripcion: 'Agencia Estatal de Energía de Hidalgo',
			},{
				id: 4,
				foto: 'img/otros/peran.gif',
				link: 'https://carnesperan.com.mx/',
				titulo: 'Distribuidora de Carnes Peran',
				descripcion: 'Distribuidora de Carnes Peran',
			},{
				id: 5,
				foto: 'img/otros/cluster.gif',
				link: 'https://clusterminerodehidalgo.mx/',
				titulo: 'Clúster Minero de Hidalgo',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 6,
				foto: 'img/otros/agc.gif',
				link: 'https://agc.com.mx/',
				titulo: 'A+ Group Consulting',
				descripcion: 'A+ Group Consulting',
			},{
				id: 7,
				foto: 'img/otros/picasso.gif',
				link: 'https://www.endopediapicasso.com.mx/',
				titulo: 'Emma Picasso',
				descripcion: 'Endocrinología Pediátrica',
			},{
				id: 8,
				foto: 'img/otros/gastos.gif',
				link: 'https://fer.agc.com.mx/gastosFer.apk',
				titulo: 'Gastos',
				descripcion: 'Control de gastos',
			},{
				id: 9,
				foto: 'img/otros/inviktus.gif',
				link: '',
				titulo: 'Grupo Invictus',
				descripcion: 'MetLife',
			},{
				id: 24,
				foto: 'img/otros/metlife.gif',
				link: 'https://metlife.grupoinviktus.com/',
				titulo: 'MetLife - Grupo Invictus',
				descripcion: 'MetLife',
			},{
				id: 23,
				foto: 'img/otros/hgo.gif',
				link: 'https://becasparalasostenibilidad.hidalgo.gob.mx/',
				titulo: 'Becas para la sostenibilidad',
				descripcion: 'Agencia Estatal de Energía de Hidalgo',
			},{
				id: 10,
				foto: 'img/otros/two.gif',
				link: '',
				//link: 'https://twobeatsound.com.mx/',
				titulo: 'TWO BEAT SOUND',
				descripcion: 'DJ',
			},{
				id: 11,
				foto: 'img/otros/kantina.gif',
				link: 'https://kantinalibre.com/',
				titulo: 'Kantina Libre',
				descripcion: 'Tequila',
			},{
				id: 12,
				foto: 'img/otros/esmagenta.gif',
				link: '',
				titulo: 'Esmagenta',
				descripcion: 'Imprenta',
			},{
				id: 13,
				foto: 'img/otros/auto.gif',
				link: 'https://autovestidurasespeciales.com.mx/',
				titulo: 'Auto Vestiduras Especiales',
				descripcion: 'Vestiduras de auto',
			},{
				id: 14,
				foto: 'img/otros/hgo.gif',
				link: 'https://sedeco.hidalgo.gob.mx/',
				titulo: 'Secretaría de Desarrollo Económico',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 15,
				foto: 'img/otros/dos.gif',
				link: 'https://dosdehidalgo.com.mx/',
				titulo: 'Dos de Hidalgo',
				descripcion: 'Noticias',
			},{
				id: 16,
				foto: 'img/otros/hgo.gif',
				link: '',
				titulo: 'Mi primer empleo',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 17,
				foto: 'img/otros/intranet.gif',
				link: '',
				titulo: 'Intranet',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 18,
				foto: 'img/otros/putrid.gif',
				link: 'https://putridscum.com.mx/',
				titulo: 'Putrid Scum',
				descripcion: 'Música',
			},{
				id: 19,
				foto: 'img/otros/soporte.gif',
				link: '',
				titulo: 'Soporte Informático',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 20,
				foto: 'img/otros/hgo.gif',
				link: '',
				titulo: 'Manos a la Obra',
				descripcion: 'Secretaría de Desarrollo Económico',
			},{
				id: 26,
				foto: 'img/otros/caminadoras.gif',
				link: 'https://servicioacaminadoras.com/',
				titulo: 'Servicio a Caminadoras y Gimnasios',
				descripcion: 'Servicio a Caminadoras y Gimnasios',
			},
		]
	},{
		id: 2,
		img: 'img/p_2.svg',
		categoria: 'Diseños',
		categoriaIN: 'Designs',
		proyecto: [
			{
				id: 1,
				foto: 'img/otros/reloj.gif',
				link: 'https://fer.agc.com.mx/img/otros/reloj_g.jpg',
				titulo: 'Reloj de Pachuca',
				descripcion: '',
			},{
				id: 2,
				foto: 'img/otros/letras.gif',
				link: 'https://fer.agc.com.mx/img/otros/letras_g.gif',
				titulo: 'Letras Magma',
				descripcion: 'Fer',
			},{
				id: 3,
				foto: 'img/otros/cuadros.gif',
				link: 'https://fer.agc.com.mx/img/otros/cuadros_g.gif',
				titulo: 'Cuadros',
				descripcion: 'Amigos',
			},{
				id: 4,
				foto: 'img/otros/lado.gif',
				link: 'https://fer.agc.com.mx/img/otros/lado_g.jpg',
				titulo: 'Doble vista',
				descripcion: 'Cara fer',
			},{
				id: 5,
				foto: 'img/otros/tecnologia.gif',
				link: 'https://fer.agc.com.mx/img/otros/tecnologia_g.jpg',
				titulo: 'Tecnología',
				descripcion: 'Tecnología',
			},{
				id: 6,
				foto: 'img/otros/abstracto.gif',
				link: 'https://fer.agc.com.mx/img/otros/abstracto_g.jpg',
				titulo: 'Abstracto',
				descripcion: 'Abstracto',
			},
		]
	},{
		id: 3,
		img: 'img/p_3.svg',
		categoria: 'Otros',
		categoriaIN: 'Others',
		proyecto: [
			{
				id: 1,
				foto: 'img/otros/agc_equipo.gif',
				link: 'https://fer.agc.com.mx/img/otros/agc_equipo_g.jpg',
				titulo: 'Equipo AGC',
				descripcion: '',
			},{
				id: 2,
				foto: 'img/otros/21k.gif',
				link: 'https://fer.agc.com.mx/img/otros/21k_g.jpg',
				titulo: '21k CDMX',
				descripcion: '',
			},{
				id: 3,
				foto: 'img/otros/unam.gif',
				link: 'https://fer.agc.com.mx/img/otros/unam_g.jpg',
				titulo: 'UNAM',
				descripcion: '',
			},{
				id: 4,
				foto: 'img/otros/malinche.gif',
				link: 'https://fer.agc.com.mx/img/otros/malinche_g.jpg',
				titulo: 'Malinche',
				descripcion: '',
			},{
				id: 5,
				foto: 'img/otros/equipo.gif',
				link: 'https://fer.agc.com.mx/img/otros/equipo_g.gif',
				titulo: 'Equipo',
				descripcion: 'Secretaría de Desarrollo Económico',
			},
		]
	},
];

window._my_servicios = [
	{
		nombre: "Administración de bases de datos",
		nombreIN: "Database administration",
		descripcion: "<ul>\n\
			<li>Realizar el backup, recuperación de base de datos y recuperación del servidor en caso de falla o para consulta posterior de la información para la continuidad del negocio</li>\n\
			<li>Realizar mejoras de rendimiento tunning con la finalidad de optimizar las base de datos o de servidores</li>\n\
			<li>Crear programas de migración de datos para los cambios solicitados de las diferentes compañías de la Institución</li>\n\
			<li>Ejecutar programas de carga para las modificaciones de estructura a tablas de la base de datos que contienen gran cantidad de registros para evitar que se queden sin recursos</li>\n\
			<li>Mantener la seguridad de la base de datos controlando el uso de las contraseñas de los usuarios actualizándolas cada cierto periodo por políticas de seguridad y a solicitud del área de Auditoría</li>\n\
			<li>Realizar la liberación a producción DDL (creación de base de datos, objetos, procedimientos) para definir las estructuras que almacenarán los datos así como los procedimientos o funciones que permitan consultarlos</li>\n\
			<li>Dar soporte a producción DML (consultas, modificación, actualización de BD) que permita a los usuarios introducir datos para realizar tareas de consultas o modificación de las Bases de Datos</li>\n\
			<li>Realizar la estandarización de nombres de objetos como son tablas, campos, índices, funciones, stored procedure, para tener un control adecuado en la base de datos</li>\n\
		</ul>",
		descripcionIN: "",
	},{
		nombre: "Desarrollo de sistemas",
		nombreIN: "Software development",	
		descripcion: "Diseño, desarrollo, implementación y soporte a soluciones tecnológicas que respondan a las necesidades de tu organización de manera ágil y eficaz.",
		descripcionIN: "",
	},{
		nombre: "Desarrollo de aplizaciones móviles",
		nombreIN: "Mobile application development",
		descripcion: "<b>Desarrollo de Aplicaciones Móviles Personalizadas</b>\n\
		<br/><br/>\n\
		Aplicaciones web, móviles e híbridas altamente escalables, flexibles e interoperables.\n\
		<br/><br/>\n\
		API personalizadas para todas las clases de aplicaciones, agregando funcionalidad a sus sistemas de software y facilitando la comunicación entre sus aplicaciones y de terceros.",
		descripcionIN: "",
	},{
		nombre: "Diseño de baners y animación",
		nombreIN: "Banner and animation design",
		descripcion: "Diseño de banners publicitarios.",
		descripcionIN: "",
	},{
		nombre: "Diseño gráfico",
		nombreIN: "Graphic design",
		descripcion: "Desde logotipos y branding hasta materiales promocionales y diseños web, te ofrecemos soluciones de diseño creativas y profesionales.",
		descripcionIN: "",
	},{
		nombre: "Diseño Web",
		nombreIN: "Web design",
		descripcion: "Soluciones de desarrollo web que faciliten los flujos de trabajo, aumenten los ingresos y optimizen las operaciones comerciales",
		descripcionIN: "",
	},{
		nombre: "Edición de vídeo",
		nombreIN: "Video edition",
		descripcion: "Servicio de videomarketing te brinda la oportunidad de transmitir tu mensaje de manera impactante a través de videos profesionales.",
		descripcionIN: "",
	},{
		nombre: "Edición y restauración de fotografías",
		nombreIN: "Editing and restoration of photographs",
		descripcion: "<ul>\n\
			<li>Retoque de piel de alta gama mediante métodos avanzados (Aspecto natural)</li>\n\
			<li>Las texturas de la piel se conservarán en el proceso</li>\n\
			<li>Eliminar imperfecciones/arrugas</li>\n\
			<li>Blanquear los dientes</li>\n\
			<li>Hacer reducción de edad</li>\n\
			<li>Colorea cualquier imagen usando Photoshop</li>\n\
			<li>Restaurar fotos antiguas o dañadas</li>\n\
			<li>Mejora de la imagen del producto (Para eBay, Amazon, Esty, etc.)</li>\n\
			<li>Retoque fotográfico de belleza/moda</li>\n\
			<li>Fotos de portada y perfil (para Fiverr, facebook, youtube, etc.)</li>\n\
			<li>Quitar/cambiar fondos</li>\n\
			<li>Reemplazar/añadir/eliminar parte de una foto</li>\n\
			<li>Manipulación de fotografías/intercambio de cabezales</li>\n\
			<li>Mejora tus fotos aburridas (Brillo/contraste/relámpago/gamma/exposición, etc.)</li>\n\
			<li>Correcciones de color</li>\n\
			<li>Cambiar tamaño/recortar imágenes</li>\n\
		</ul>",
		descripcionIN: "",
	},{
		nombre: "Impresión en serigrafía",
		nombreIN: "Serigraphy",
		descripcion: "",
		descripcionIN: "",
	},{
		nombre: "Redes sociales",
		nombreIN: "Social networks",
		descripcion: "Conecta con tu audiencia, domina nuevas estrategias e involucra a tu comunidad en las redes sociales.",
		descripcionIN: "",
	},{
		nombre: "Soporte técnico",
		nombreIN: "Technical support",
		descripcion: "Soporte integral, que cubre todas las necesidades de tu empresa. Nuestros servicios abarcan áreas clave como usuarios, hardware, software, redes informáticas, servidores y respaldo de datos.",
		descripcionIN: "",
	},{
		nombre: "Sublimación en artículos",
		nombreIN: "Sublimation on products",
		descripcion: "",
		descripcionIN: "",
	},{
		nombre: "Vectorización de logos e imagenes",
		nombreIN: "Vectorization of logos and images",
		descripcion: "",
		descripcionIN: "",
	},{
		nombre: "Venta y/o cotización de equipo de computo",
		nombreIN: "Sale and/or quote of computer equipment",
		descripcion: "Asesoramiento en equipo de computo, caracteristicas, costos, beneficios etc.",
		descripcionIN: "",
	},{
		nombre: "Ventas en linea (Market place, Mercado Libre, Amazon)",
		nombreIN: "Online sales (Market place, Mercado Libre, Amazon)",
		descripcion: "Apertura de tu cuenta, carga de tu catálogo, configuración de la Logística del marketplace, promociones, y construcción de tus primeras campañas de publicidad, hasta capacitarte para que puedas continuar con tu operación de forma independiente.",
		descripcionIN: "",
	},
];
